...
def clickclassrep():
    ...
    sns.countplot(data=TitanicData, palette="winter")
    plt.show()
...
# Classification report
classrep = Button(wdw, text="Classification Report", bg="black", fg="white",
                   font="none 16 bold", width=17, command=clickclassrep)
classrep.grid(row=0, column=6, sticky=W)
classrep.bind("<Enter>", hover_in)
classrep.bind("<Leave>", hover_out)


# Button Exit
quit = Button(wdw, text="EXIT", bg="grey", fg="white", font="none 20 bold", width=10, height=5, command=closewdw)
